...
#ifndef LLVM_EXECUTIONENGINE_ORC_COMPILEONDEMANDLAYER_H_ADAPT
#define LLVM_EXECUTIONENGINE_ORC_COMPILEONDEMANDLAYER_H_ADAPT
...
class AdaptiveCompileOnDemandLayer {
private:
  ...
  template <typename ResourceT> class ResourceOwner {
  public:
...
public:
  ...
  AdaptiveCompileOnDemandLayer(
      BaseLayerT &BaseLayer, PartitioningFtor Partition,
  ...
  ModuleSetHandleT addModuleSet(ModuleSetT Ms, MemoryManagerPtrT MemMgr,
    ...
    LD.ModuleAdder = [&MemMgrRef](BaseLayerT &B, std::unique_ptr<Module> M,
  ...
  void removeModuleSet(ModuleSetHandleT H) { LogicalDylibs.erase(H); }
  ...
  JITSymbol findSymbolIn(ModuleSetHandleT H, const std::string &Name,
                         bool ExportedSymbolsOnly) {
    // errs() << "Symbol searching for: " << Name << "\n";
  ...
  void addLogicalModule(LogicalDylib &LD, ModulePtrT SrcMPtr) {
    ...
    const DataLayout &DL = SrcM.getDataLayout();
    {
      ...
      for (auto &F : SrcM) {
        ...
        if (auto Sym = LD.StubsMgr->findStub(F.getName(), false)) {
          errs() << "this was actually the problem lol?\n";
          F.setName(F.getName() + Twine(LD.dup_count++));
        }
        ...
        StubInits[MangledName] = std::make_pair(
            CCInfo.getAddress(), JITSymbolFlags::fromGlobalValue(F));
      ...
      /*if (auto Sym = LD.StubsMgr->findStub("init", false))
      {
        if (StubInits.find("init") != StubInits.end()) {
            errs() << "Tarun! we are duplicating init another init\n";
            errs() << "Tarun! Addr: " << Sym.getAddress() << "\n";
      ...
      if (auto Sym = LD.StubsMgr->findStub("init", false))
      {
        errs() << "Tarun! New Init Addr: " << Sym.getAddress() << "\n";
      }*/
    ...
    auto Materializer =
        createLambdaMaterializer([this, &LD, &GVsM](Value *V) -> Value * {
          if (auto *F = dyn_cast<Function>(V)) {
            ...
            ConstantInt *StubAddr = ConstantInt::get(
                GVsM->getContext(), APInt(PtrBitWidth, StubSym.getAddress()));
            Constant *Init = ConstantExpr::getCast(Instruction::IntToPtr,
                                                   StubAddr, F->getType());
            return GlobalAlias::create(
                F->getFunctionType(), F->getType()->getAddressSpace(),
                F->getLinkage(), F->getName(), Init, GVsM.get());
          }
          // else....
          return nullptr;
        });
    ...
    for (auto &A : SrcM.aliases()) {
      ...
      Value *Init =
          MapValue(A.getAliasee(), VMap, RF_None, nullptr, &Materializer);
    ...
    auto GVsH =
        LD.ModuleAdder(BaseLayer, std::move(GVsM), std::move(GVsResolver));
...
#endif // LLVM_EXECUTIONENGINE_ORC_COMPILEONDEMANDLAYER_H_ADAPT
