...
void StaticBinaryCache::notifyObjectCompiled(const Module *M,
  ...
  DEBUG(dbgs() << "Copying binary for " << M->getModuleIdentifier()
	<< " from " << execFilePath << " to " << CacheName << "\n");
...
std::unique_ptr<MemoryBuffer> StaticBinaryCache::getObject(StringRef Name) const {
  std::string CacheName;
...
int StaticBinaryCache::getObjectFileDesc(StringRef Name) const {
  ...
  if (execFD == -1) {		// Not an error: just a cache miss
    DEBUG(dbgs() << "Executable does not exist in file cache "
	  << CacheDir << "\n");
  }
